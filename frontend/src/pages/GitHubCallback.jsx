...
const GitHubCallback = () => {
  ...
  const hasRun = useRef(false);  // ✅ Thêm ref để track đã chạy chưa
  ...
  useEffect(() => {
    ...
    const handleGitHubCallback = async () => {
      try {
        ...
        if (errorParam) {
          console.error('User denied GitHub authorization');
        ...
        const tokenResponse = await fetch('http://localhost:8081/api/auth/github/exchange-token', {
          method: 'POST',
          headers: { 
