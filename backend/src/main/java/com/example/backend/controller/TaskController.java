...
public class TaskController {
    ...
    /**
     *  Quick update task status - SEPARATE METHOD
     * Fast endpoint to change task status (TODO/IN_PROGRESS/DONE)
     * Uses dedicated service method with proper permission check
     *
     * @param userDetails authenticated user details
     * @param taskId ID of the task
    ...
    public ResponseEntity<ApiResponse<TaskDetailResponse>> updateTaskStatus(
        ...
        try {
            TaskDetailResponse response = taskService.updateTaskStatus(
                    userDetails.getUsername(), taskId, status);
    ...
    /**
     *  Quick update task priority - SEPARATE METHOD
    ...
    public ResponseEntity<ApiResponse<TaskDetailResponse>> updateTaskPriority(
        ...
        try {
            TaskDetailResponse response = taskService.updateTaskPriority(
                    userDetails.getUsername(), taskId, priority);
     ...
     * Retrieves tasks where the current user is an assignee
     * Supports filtering by status and priority
     *
     * @param userDetails authenticated user details
     * @param status optional status filter (TODO/IN_PROGRESS/DONE)
     * @param priority optional priority filter (LOW/MEDIUM/HIGH/URGENT)
     * @param page page number (starts from 0)
     * @param size number of items per page
    ...
    public ResponseEntity<ApiResponse<PageResponse<TaskListResponse>>> getMyTasks(
        ...
        log.info("[TaskController] Getting my tasks | User: {} | Status: {} | Priority: {} | Page: {} | Size: {}",
                userDetails.getUsername(), status, priority, page, size);

        try {
            PageResponse<TaskListResponse> response = taskService.getMyTasks(
                    userDetails.getUsername(), status, priority, page, size);
            log.info("[TaskController] My tasks retrieved successfully | User: {} | ResultCount: {}",
                    userDetails.getUsername(), response.getTotalElements());
            return ResponseEntity.ok(ApiResponse.success("Lấy danh sách tasks thành công", response));
